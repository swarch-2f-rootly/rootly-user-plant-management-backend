...
from app.core.ports.repositories import DeviceRepositoryPort
from app.adapters.repositories import models
from app.core.domain.entities import Microcontroller, UserMicrocontroller, Plant
...
class SQLAlchemyDeviceRepository(DeviceRepositoryPort):
    ...
    def user_has_permission(self, user_id: UUID, device_id: UUID, min_role="viewer", user_claims=None):
        ...
        if not assoc:
            return False
        return ROLE_LEVEL.get(assoc.role, 0) >= ROLE_LEVEL.get(min_role, 0)# Implementación de repositorio usando SQLAlchemy
class SQLAlchemyDeviceRepository(DeviceRepositoryPort):
    def __init__(self, db_session):
        self.db = db_session
